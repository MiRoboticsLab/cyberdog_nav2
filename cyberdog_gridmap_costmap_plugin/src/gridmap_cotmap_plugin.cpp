  ...
  grid_map_msgs::msg::GridMap::ConstSharedPtr message)
{
  ...
  RCLCPP_DEBUG(
    rclcpp::get_logger(
      "nav2_costmap_2d"),
    "ScanCallback : position: (%f, %f);  resolution %f", mapPosition_.x(),
    mapPosition_.y(), resolution_);

  RCLCPP_DEBUG(
    rclcpp::get_logger(
      "nav2_costmap_2d"),
    "             : mapLength_:(%f, %f);  bufferSize_ (%d, %d)", mapLength_.x(),
    mapLength_.y(), bufferSize_(0), bufferSize_(1));
