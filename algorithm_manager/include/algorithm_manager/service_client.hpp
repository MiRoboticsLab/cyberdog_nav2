...
#ifndef ALGORITHM_MANAGER__SERVICE_CLIENT_HPP_
#define ALGORITHM_MANAGER__SERVICE_CLIENT_HPP_
...
public:
  ...
  */
    ...
    const std::chrono::nanoseconds timeout = std::chrono::nanoseconds(-1))
  {
    ...
    INFO("%s service client: send async request finished", service_name_.c_str());

    if (callback_group_executor_.spin_until_future_complete(future_result, timeout) !=
      rclcpp::FutureReturnCode::SUCCESS)
    {
      ERROR("%s service client: async_send_request failed", service_name_.c_str());
      throw std::runtime_error(service_name_ + " service client: async_send_request failed");
    }

    auto result = future_result.get();
    INFO("[---->] auto result = future_result.get()");
    return result;
  }
    ...
    const int timeout)
  {
    ...
    INFO("%s service client: send async request with timeout", service_name_.c_str());
    auto future_result = client_->async_send_request(request);
  ...
  */
    ...
    typename ResponseType::SharedPtr & response)
  {
    ...
    INFO("%s service client: send async request without timeout", service_name_.c_str());
    auto future_result = client_->async_send_request(request);
...
#endif  // ALGORITHM_MANAGER__SERVICE_CLIENT_HPP_
