...
bool AutomaticRechargeNavigator::configure(
  ...
  if (!node->has_parameter("goal_blackboard_keep_dist")) {
    node->declare_parameter("goal_blackboard_keep_dist",
  ...
  goal_sub_ = node->create_subscription<geometry_msgs::msg::PoseStamped>(
      ...
      std::bind(&AutomaticRechargeNavigator::onGoalPoseReceived, this,
                std::placeholders::_1));
  return true;
}
...
std::string AutomaticRechargeNavigator::getDefaultBTFilepath(
  ...
  node->get_parameter("default_automatic_recharge_bt_xml",
