...
bool
AutomaticRechargeNavigator::configure(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  start_time_ = rclcpp::Time(0);
  auto node = parent_node.lock();
  if (!node->has_parameter("charging_station")) {
    node->declare_parameter("charging_station", std::string("charging_station"));
  }
  ...
  goal_sub_ = node->create_subscription<geometry_msgs::msg::PoseStamped>(
    ...
    std::bind(&AutomaticRechargeNavigator::onGoalPoseReceived, this, std::placeholders::_1));
  return true;
}

std::string
AutomaticRechargeNavigator::getDefaultBTFilepath(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  ...
  node->declare_parameter<std::string>(
    ...
    pkg_share_dir +
    "/behavior_trees/automatic_recharge.xml");
  node->get_parameter("default_automatic_recharge_bt_xml", default_bt_xml_filename);

  return default_bt_xml_filename;
}

bool
AutomaticRechargeNavigator::cleanup()
{
  goal_sub_.reset();
  self_client_.reset();
  return true;
...
bool
AutomaticRechargeNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
...
void
AutomaticRechargeNavigator::goalCompleted(typename ActionT::Result::SharedPtr /*result*/)
{
}

void
AutomaticRechargeNavigator::onLoop()
{
...
void
AutomaticRechargeNavigator::onPreempt(ActionT::Goal::ConstSharedPtr goal)
{
...
void
AutomaticRechargeNavigator::initializeGoalPose(ActionT::Goal::ConstSharedPtr goal)
{
...
void
AutomaticRechargeNavigator::onGoalPoseReceived(
