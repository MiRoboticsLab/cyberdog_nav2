...
BtActionServer<ActionT>::BtActionServer(
  ...
  RCLCPP_ERROR(logger_, "BtActionServer  :%s", action_name.c_str());
...
bool BtActionServer<ActionT>::on_configure() {
  ...
  auto options = rclcpp::NodeOptions().arguments(
      {"--ros-args", "-r",
       std::string("__node:=") + std::string(node->get_name()) +
  ...
  RCLCPP_ERROR(logger_, "create action server name 1 :%s",
               action_name_.c_str());

  // add poses subscriber.
  follow_poses_ = node->create_subscription<FollowPoses>(
...
bool BtActionServer<ActionT>::on_activate() {
  RCLCPP_ERROR(logger_, "0 on_activate :%s", action_name_.c_str());
  if (!loadBehaviorTree(default_bt_xml_filename_)) {
    RCLCPP_ERROR(logger_, "Error loading XML file: %s",
                 default_bt_xml_filename_.c_str());
    return false;
  }

  RCLCPP_ERROR(logger_, "on_activate :%s", action_name_.c_str());
...
bool BtActionServer<ActionT>::loadBehaviorTree(
  ...
  if (enable_groot_monitoring_) {
    ...
    try {
      bt_->addGrootMonitoring(&tree_, groot_zmq_publisher_port_,
                              groot_zmq_server_port_);
