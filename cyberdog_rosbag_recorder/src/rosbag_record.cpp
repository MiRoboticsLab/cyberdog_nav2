...
TopicsRecorder::TopicsRecorder() : Node("rosbag_snapshotter")
    , rosbag_file_path_(kROSBagDirectory)
{
    server_ = create_service<std_srvs::srv::SetBool>("rosbag_snapshot_trigger",
        std::bind(&TopicsRecorder::SnapshotServiceCallback, this, 
...
std::vector<std::string> TopicsRecorder::GetParams()
{
    ...
    std::string namespace_topic;

    for (auto topic: topics) {
        // if (topic[0] != '/') {
        //     namespace_topic = this->get_namespace() + std::string("/") + topic;
        // } else {
